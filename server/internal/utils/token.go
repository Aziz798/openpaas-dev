...
func GenerateToken(userID uint, userRole string, isPremium bool) (string, string, error) {
	// Access Token
...
func RefreshToken(tokenString string) (string, string, error) {
	...
	userID, ok := claims["user_id"].(float64) // JWT stores numeric values as float64, so it needs conversion
	if !ok {
	...
	// Extract user_premium from the claims
	userPremium, ok := claims["premium"].(bool)
	if !ok {
		return "", "", fmt.Errorf("invalid token payload")
	}
	...
	accessClaims := jwt.MapClaims{
		"user_id": uint(userID),                            // Convert float64 back to uint
		"exp":     time.Now().Add(time.Minute * 15).Unix(), // Token valid for 15 minutes
		"role":    userRole,
		"premium": userPremium,
	}
